...
// ─── Cancel Dialog ───────────────────────────────────────────────
const CancelDialog = ({ order, loading, onCancel, onConfirm }) => {
...
function NewOrderModal({ shopProducts, onClose, onSuccess }) {
  const [cart, setCart] = useState([]); // { product, name, sku, unitPrice, quantity, stock }
...
export default function StaffDashboard() {
  ...
  useEffect(() => {
    Promise.all([fetchShop(), fetchStats()]).finally(() => setLoading(false));
  }, []);
      ...
      >
        ...
        {orderLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p style={{ color: "var(--text-primary)" }}>No orders yet</p>
            <button
              onClick={() => setShowNewOrder(true)}
                        ...
                        style={{
                          ...
                          color:
                            item.allocatedQuantity === 0
                              ? "var(--danger-text)"
                              : "#4ade80",
                          border: `1px solid ${item.allocatedQuantity === 0 ? "var(--danger-border)" : "rgba(74,222,128,0.25)"}`,
                        }}
                      >
